...
# To use a different branch, change revisio
#lm = AutoModelForCausalLM.from_pretrained(, model_file="codellama-7b-instruct.q4_K_M.gguf", model_type="llama", gpu_layers=50)
...
prompt_template=f'''[INST] use javascript
{prompt}
[/INST]
...
#rint(makeFunctionFromText('make a canvas and draw a bunch of triangles')[0]['generated_text'])
...
#write string
#gen code -> run in serverless function or on server as module-import/eval
#type check - or use acorn-walk to find arguments and then
#

#watch for tweets that mention pizza memes
#when one happens, download and then make a song from the text using wavenet
#when a tweet with #pizza happens, make an mp3 if the text is longer than 30 tokens

#document = [one, two, three]
#code = [one, two, three]
#dependencies + results
#when there is a sentence edit, rerun that function instantly
#only re-run other functions if their data has changed

cache = {}
def two (parameters):
    return 123

#reactive variables client/server
#only python
#line = watch for tweets that mention #pizza
#re-run functions every 5 min and if the data has changed,
#client polls server every 5 min and if data has changed, then run all other functions in document
...
async def index():
    result = processMessages(messages)
    return JSONResponse(content={'nexus': 1})

from fastapi import FastAPI, Query

@app.post("/makeFn/{Fn}")
async def makeFn(Fn:str):
    # print(Fn)
    # fn = makeFunctionFromText(Fn)[0]['generated_text']
    # print('something else')
    # return {'fn': fn}
    sentences = document.split('\n')
    fn = [makeFunctionFromText(sentence) for sentence in document]
    result = False
    for f in fn:
