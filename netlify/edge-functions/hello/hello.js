...
const handler = async (event) => {
  ...
  //let documents = await updateCell(auth, 'Sheet1!L:L', []) // write the document post cluster 
...
export default async (req, context) => {
  ...
  let documents = await updateCell(auth, 'Sheet1!Q:Q', message)

  const body = await req.json();

  console.log(body)
  return {statusCode: 200, body: JSON.stringify({ message: `Hello ${topics}` })}
